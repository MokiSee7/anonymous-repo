#!/usr/bin/env python3
"""
Generate OVP H5 File from Scratch

This script creates the OVP H5 file by:
1. Sampling video frames at 2 FPS
2. Reading UserSummary annotations (5 users)
3. Computing gtscore for each sampled frame
4. Generating gtsummary (top 15% frames)
5. Running KTS algorithm to get change_points
6. Running Knapsack algorithm to get shot_level_gt

Input:
    - datasets_preprocessed/data/dataset/OVP/video/*.mpg
    - datasets_preprocessed/data/dataset/OVP/UserSummary/*

Output:
    - datasets_preprocessed/data/h5/ovp.h5
    - datasets_preprocessed/data/dataset/OVP/frames/video_*/*.jpg
...
def extract_frame_features(frames_dir, clip_model, preprocess, device):
    ...
    Returns:
        features: (n_frames, feature_dim) array of features
    """
...
def sample_video_frames(video_path, output_dir, target_fps=2.0):
    ...
    for i, frame_idx in enumerate(tqdm(picks, desc="   Extracting", leave=False)):
        ...
        # Format: frame_00000.jpg, frame_00015.jpg, etc.
...
def read_user_summary(user_summary_dir, video_id, picks):
    ...
    for user_idx, user_dir in enumerate(user_dirs):
        ...
        frame_files = list(user_dir.glob('Frame*.jpeg'))

        selected_frames = []
        for frame_file in frame_files:
            # Extract frame number from filename (e.g., "Frame1081.jpeg" -> 1081)
            match = re.search(r'Frame(\d+)\.jpeg', frame_file.name)
            if match:
        ...
        for pick_idx, pick_frame in enumerate(picks):
            ...
            # Allow tolerance of ±sample_interval/2
...
def process_ovp_dataset(video_dir, user_summary_dir, frames_dir, output_h5_path, target_fps=2.0,
                        extract_features=True, clip_model_name='ViT-B/32'):
    """
    Process all OVP videos and create H5 file
    ...
    video_files = sorted(video_dir.glob('v*.mpg'))
    ...
    with h5py.File(output_h5_path, 'w') as h5_file:
        for video_file in tqdm(video_files, desc="Processing videos"):
            ...
            try:
                ...
                video_id = video_file.stem  # e.g., "v21"
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate OVP H5 file from videos and UserSummary annotations',
    ...
    default_user_summary_dir = project_root / "datasets_preprocessed/data/dataset/OVP/UserSummary"
    default_frames_dir = project_root / "datasets_preprocessed/data/dataset/OVP/frames"
    default_output_h5 = project_root / "datasets_preprocessed/data/h5/ovp.h5"
