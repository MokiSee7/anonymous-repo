"""
exp5_direct_alignment: Direct multimodal clip-to-sentence alignment with GPT-5.2 / GPT-4.1-mini.
...
Usage (conda env: exp7):
    ...
    python run_gpt.py --model gpt-4.1-mini --dataset summe
...
def build_messages(shots, shuffled_sentences):
    ...
    per_clip_limit = max(1, AZURE_MAX_IMAGES // n_clips) if n_clips > 0 else FRAMES_MAX
    for i, shot in enumerate(shots):
        k = min(compute_n_frames_for_shot(len(shot["frames"])), per_clip_limit)
    ...
    return messages, sum(n_capped_per_shot)
...
def _infer(client, model_name, shots, shuffled_sentences):
    out = {"text": "", "n_input_tokens": None, "n_output_tokens": None,
           "total_frames_submitted": None, "error": None}
    try:
        messages, total_frames = build_messages(shots, shuffled_sentences)
...
def run_video(client, model_name, dataset_name, video_id, seed):
    ...
    r = _infer(client, model_name, shots, shuffled_sents)
...
def main():
    ...
    print(f"\nRunning {args.model} | dataset={args.dataset} ({len(video_ids)} videos)\n")
