"""
exp4_temporal_understanding: Temporal reordering with GPT-5.2 / GPT-4.1-mini.

Multimodal version: actual video frames submitted via Azure OpenAI vision API.
All settings (shot/sentence selection, RNG, evaluation) are identical to run_qwen.py.
...
Usage (conda env: exp7):
    ...
    python run_gpt.py --model gpt-5.2 --dataset summe --no_doc
    python run_gpt.py --model gpt-5.2 --dataset summe --mode sentence
...
def _parse_ranking(text, n, primary_pattern, label_name):
    """
    Generic ranking parser. Returns (ranking, failure_reason).
    Primary: "<label_name> X > <label_name> Y > ..."
    Fallback: pure numbers "X > Y > ..."
...
def build_messages_shot_reorder(shuffled_shot_frames, document_text):
...
def build_messages_sentence_reorder(shuffled_sentences, frames):
...
def build_messages_sentence_text_only(shuffled_sentences):
    n = len(shuffled_sentences)
    content = [{"type": "text", "text": f"Below are {n} sentences describing events in a video, shown in shuffled order:\n"}]
    for i, sent in enumerate(shuffled_sentences):
        content.append({"type": "text", "text": f"Sentence {i + 1}: {sent}\n"})
...
def _infer(client, model_name, messages):
...
def run_video_shot(client, model_name, dataset_name, video_id, rng, with_doc=True):
    ...
    total_raw = sum(len(f) for _, f in shot_frames_list)
    if total_raw > AZURE_MAX_IMAGES:
        per_shot = max(1, AZURE_MAX_IMAGES // len(shot_frames_list))
        capped = []
        for si, fs in shot_frames_list:
            if len(fs) > per_shot:
                idxs = [int(i * len(fs) / per_shot) for i in range(per_shot)]
                fs = [fs[i] for i in idxs]
            capped.append((si, fs))
        shot_frames_list = capped

    n_frames_per_shot = [len(f) for _, f in shot_frames_list]
    n_frames_original = sum(n_frames_per_shot)

    document_text = load_full_document_text(dataset_name, video_id) if with_doc else None
    messages = build_messages_shot_reorder(shot_frames_list, document_text)
    infer_out = _infer(client, model_name, messages)
    ...
    if infer_out["error"]:
        ...
        print(f"  in={infer_out['n_input_tokens']}  out={infer_out['n_output_tokens']}  frames={n_frames_original}")
...
def run_video_sentence(client, model_name, dataset_name, video_id, text_only, rng):
    sentences = load_sentences(dataset_name, video_id)
    if not sentences:
    ...
    if text_only:
        ...
        messages = build_messages_sentence_text_only(shuffled_sentences)
    else:
        meta = load_video_meta(dataset_name, video_id)
        n_shots_total = len(meta["n_frame_per_seg"])
        ...
        messages = build_messages_sentence_reorder(shuffled_sentences, all_frames)
...
def main():
    ...
    stem = f"{args.model}_{args.dataset}_{mode_tag}_seed{args.seed}"
    ...
    print(f"\nRunning {args.model} on {args.dataset} "
    ...
    for i, vid in enumerate(video_ids):
        ...
        for attempt in range(1, 4):
            ...
            try:
                if args.mode == "shot":
                    result = run_video_shot(client, args.model, args.dataset, vid, rng,
                                            with_doc=not args.no_doc)
                else:
                    result = run_video_sentence(client, args.model, args.dataset, vid,
                                                args.text_only, rng)
                break
            except Exception as e:
                last_exc = e
