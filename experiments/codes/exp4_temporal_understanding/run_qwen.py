"""
exp4_temporal_understanding: Temporal reordering with Qwen2.5-VL / Qwen3-VL.
...
Features:
  ...
  - Fine-grained parse failure tracking (empty_output / wrong_format / duplicated_items / missing_items)
  - Fallback parser: accepts pure-number format (e.g., "3 > 1 > 4 > 2 > 5")
...
Usage:
    ...
    python run_qwen.py --model qwen2.5 --dataset summe --mode shot --no_doc
    python run_qwen.py --model qwen2.5 --dataset summe --mode sentence
    python run_qwen.py --model qwen2.5 --dataset summe --mode sentence --text_only
"""
...
def build_content_sentence_reorder(shuffled_sentences, frames):
    """Sentence reordering with SENTENCE_FRAMES_PER_SHOT representative frames per shot."""
