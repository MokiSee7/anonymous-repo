"""
exp5_hierarchical_alignment: Two-step hierarchical alignment with VideoLLaMA3-7B.
...
Two-step process per video:
  ...
  Step 2 — Visual:
    Input:  All video clips (flat frame list) with clip labels described in text +
...
Metrics:
  ...
  Final chain (clips): precision, recall, f1, iou, hit@any
  Oracle chain (clips): same set (upper bound)
...
# ── Prompt builders ────────────────────────────────────────────────────────────
def build_step1_text(doc_sents, shuffled_sents):
...
def build_step2_prompt_and_frames(shots, doc_indices_0indexed, doc_sents):
    """
    Build text prompt + flat frame list for step 2.
    ...
    if not doc_indices_0indexed:
        prompt_text = (
            ...
            f"organized into {n_clips} clips as follows:\n"
            + "\n".join(clip_labels) + "\n\n"
    ...
    prompt_text = (
        ...
        f"organized into {n_clips} clips as follows:\n"
        + "\n".join(clip_labels) + "\n\n"
        f"Here are {n_cands} candidate document sentence(s) to match to the clips above:\n\n"
        f"{cand_lines}"
        f"{instruction}"
    )
    return prompt_text, all_frames
...
def _infer_visual(model, processor, frames, text):
    out = {"text": "", "n_output_tokens": None, "error": None}
    try:
        conversation = [
            {"role": "system", "content": "You are a helpful assistant."},
...
def _run_step2(model, processor, shots, doc_indices_0indexed, doc_sents, label="step2"):
    if not doc_indices_0indexed:
        return {
            ...
            "parse_ok": None, "total_frames_submitted": None, "doc_to_clips": {},
        }
    prompt_text, all_frames = build_step2_prompt_and_frames(shots, doc_indices_0indexed, doc_sents)
    total_frames = len(all_frames)
    print(f"  [{label}] cands={len(doc_indices_0indexed)}, frames_submitted={total_frames}")

    r = _infer_visual(model, processor, all_frames, prompt_text)
    if r["error"]:
        print(f"  [{label}] ERROR: {r['error'][:100]}")
        return {
            "skipped": False, "raw_output": None,
            "n_output_tokens": None, "error": r["error"],
            "format_ok": None, "nonempty_predictions_count": None, "failure_reasons": None,
            "parse_ok": None, "total_frames_submitted": total_frames, "doc_to_clips": {},
        }
...
def run_video(model, processor, dataset_name, video_id, seed):
    ...
    if not summ_sents or not doc_sents:
        ...
        return {
            ...
            "step1_result": {}, "step2_result": {}, "step2_oracle_result": {},
            "sentence_results": [], **_null_metrics(), "error": "no_data",
        }
    ...
    # Step 1: text-only
    step1_text = build_step1_text(doc_sents, shuffled_sents)
    r1 = _infer_text(model, processor, step1_text)
...
def main():
    ...
    stem     = f"hierarchical_videollama3_{args.dataset}_seed{args.seed}_{timestamp}"
    ...
    print(f"  Step1 F1={fmt(avg_s1_f1)}")
    print(f"  Final F1={fmt(avg_fn_f1)}  IoU={fmt(avg_fn_iou)}  Hit={fmt(avg_fn_hit)}")
    print(f"  Oracle F1={fmt(avg_orc_f1)}")
